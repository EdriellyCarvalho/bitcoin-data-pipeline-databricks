...
# MAGIC %md
# MAGIC # Extração de Dados Bitcoin
# MAGIC
# MAGIC - Projeto: Análise de Cotação do Bitcoin
# MAGIC - Plataforma: Databricks (Free Edition)
# MAGIC - Descrição: Pipeline de dados com extração via API, tratamento,
# MAGIC - persistência em tabela e análise via SQL e dashboard.
# MAGIC
...
# Criar widget para receber parêmetros do pipeline (key-value)
...
def extrair_cotacao_usd_brl():
    ...
    url = f'https://api.currencyfreaks.com/v2.0/rates/latest?apikey=70ed6693646641ad93da37d33275cd1e'
    resposta = requests.get(url)
    return resposta.json()
...
# MAGIC COMMENT 'Catálogo criado para o workshop de pipeline_api_bitcoin';
